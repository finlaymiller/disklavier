...
class MusicPatchilizer:
    ...
    def encode(
        self, music, music_length, patch_length=PATCH_LENGTH, add_eos_patch=False
    ):
        """
...
class MusicEncoder(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.patch_embedding = torch.nn.Linear(
            PATCH_LENGTH * PATCH_FEATURES, config.hidden_size
        )
    ...
    def forward(self, input_musics, music_masks):
        ...
        input_musics = torch.nn.functional.one_hot(
            input_musics, num_classes=PATCH_FEATURES
        )

        # Reshape the input music patches to feed into the linear layer
        input_musics = input_musics.reshape(
            len(input_musics), -1, PATCH_LENGTH * PATCH_FEATURES
        ...
        output = self.enc(
            inputs_embeds=input_musics, attention_mask=music_masks.to(self.device)
        )
...
class CLaMP(PreTrainedModel):
    ...
    def forward(self, input_texts, text_masks, input_musics, music_masks):
        ...
        """
        # Encode input texts
        text_features = self.text_enc(
            input_texts.to(self.device), attention_mask=text_masks.to(self.device)
        )["last_hidden_state"]
