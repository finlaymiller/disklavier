...
class PianoRollView(QGraphicsView):
    ...
    processed_transitions: List[Tuple[float, bool]] = []
    ...
    def update_transitions(self, transitions_data: List[Tuple[float, bool]]):
        self.processed_transitions = []
        for t_sec, is_new_track_flag in transitions_data:
            self.processed_transitions.append((t_sec * 1000, is_new_track_flag))
        ...
        for i, (start_ms, is_new_track_here) in enumerate(self.processed_transitions):
            if is_new_track_here:
                ...
                if i + 1 < len(self.processed_transitions):
                    end_ms_of_this_segment = self.processed_transitions[i + 1][0]
                else:
    ...
    def draw_transition_lines(self, painter):
        ...
        for (
            transition_time_ms,
            _,
        ) in self.processed_transitions:
            x = self.time_to_x(transition_time_ms)
